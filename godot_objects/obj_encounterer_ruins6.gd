# Auto-converted from GameMaker: obj_encounterer_ruins6
...
func _ready():
	alldead= 0
	GS.encounter= 0
	script_execute(101/* scr_steps */, 140, 120, 20, 202)

func _process(delta: float):
	if(GS.encounter > steps) {
	    if(GS.phasing == 0) {
	        if(GS.interact == 0) {
	            script_execute(101/* scr_steps */, 140, 210, 20, 202)
	            if(GS.flag[6] == 1) script_execute(101/* scr_steps */, 120, 120, 20, 202)
	            GS.encounter= 0
