# Auto-converted from GameMaker: obj_encounter_ruins2
...
func _ready():
	alldead= 0
	GS.encounter= 0
	script_execute(101/* scr_steps */, 260, 140, 20, 202)

func _process(delta: float):
	if(GS.encounter > steps and GS.interact == 0) {
	    script_execute(101/* scr_steps */, 250, 230, 20, 202)
	    GS.encounter= 0
	    battlegroup= random(20)
	    if(battlegroup <= 5) GS.battlegroup= 4
	    if(battlegroup > 5) GS.battlegroup= 5
	    if(battlegroup > 10) GS.battlegroup= 7
	    if(battlegroup > 15) GS.battlegroup= 6
	    if(battlegroup > 18) GS.battlegroup= 9
	    if(GS.flag[6] == 1) {
