# Auto-converted from GameMaker: obj_shopkeeper_catty
extends Node2D
...
func _on_outside_room():
	// obj_shop3
	with(1411) {
	    if(is_instance_valid(cattywriter) and obj_shopkeeper_catty.face == 0) {
	...
	if(face > 0)
	    draw_sprite(862/* spr_cattyface */, face, x + 34 + floor(bxx), y + 17 + floor(byy))
	armexp= GS.faceemotion
	if(armexp == 0) {
	    face= 0
	    idealarmx1= x + 60
	    idealarmy1= 130
	...
	draw_sprite(863/* spr_catarm */, arm, armx1 + xx, army1 + yy)
	draw_sprite_ext(863/* spr_catarm */, arm, armx2 + xx, army2 + yy, -1, 1, 0, 16777215, 1)
