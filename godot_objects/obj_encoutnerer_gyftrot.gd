# Auto-converted from GameMaker: obj_encoutnerer_gyftrot
...
func _ready():
	alldead= 0
	GS.encounter= 0
	if(GS.entrance == 1) script_execute(101/* scr_steps */, 160, 30, 16, 203)
	else  script_execute(101/* scr_steps */, 300, 30, 16, 203)
	...
	if(GS.encounter > steps) {
	    if(GS.phasing == 0) {
	        if(GS.interact == 0) {
	            ...
	            if(battlegroup >= 0) GS.battlegroup= 28
	            if(battlegroup > 8) GS.battlegroup= 28
	            if(GS.flag[70] < 3) {
	                GS.battlegroup= 24
	                GS.flag[70]= 4
	            }
