# Auto-converted from GameMaker: obj_battlerquick
...
func _gm_event_2_4():
	...
	on= 0
	if(clap > 2) {
	    j= instance_create(obj_mainchara.x + 5, obj_mainchara.y + 17, 141/* obj_transheart */)
	    j.mode= 1
	    heartdraw= 0
	    obj_mainchara.z_index= 100
	} else  $Alarm4.start((1) / 30.0)

func _gm_event_2_3():
	GS.currentroom= room
	room_persistent= 1
	f= instance_create(0, 0, 147/* obj_fader */)
	f.tspeed= -0.25
	get_tree().change_scene_to_file("res://godot_rooms/myroom.tscn")
