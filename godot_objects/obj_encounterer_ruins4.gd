# Auto-converted from GameMaker: obj_encounterer_ruins4
...
func _ready():
	alldead= 0
	GS.encounter= 0
	script_execute(101/* scr_steps */, 90, 100, 20, 202)

func _process(delta: float):
	if(GS.encounter > steps and GS.interact == 0) {
	    script_execute(101/* scr_steps */, 120, 100, 20, 202)
	    if(GS.flag[6] == 1) script_execute(101/* scr_steps */, 120, 60, 20, 202)
	    GS.encounter= 0
	    battlegroup= random(20)
	    if(battlegroup <= 4) GS.battlegroup= 11
	    if(battlegroup > 4) GS.battlegroup= 12
	    if(battlegroup > 8) GS.battlegroup= 13
	    if(battlegroup > 12) GS.battlegroup= 16
	    if(battlegroup > 16) GS.battlegroup= 17
	    if(battlegroup > 19) GS.battlegroup= 15
	    if(GS.flag[6] == 1) {
