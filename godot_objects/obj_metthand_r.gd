# Auto-converted from GameMaker: obj_metthand_r
...
func _on_outside_room():
	draw_set_color(0)
	draw_rectangle(segx + 2, y + 2, GS.idealborder[0] - 2, y + 18, 0)
	draw_set_color(16777215)
	draw_rectangle(segx, y + 4, GS.idealborder[0], y + 16, 0)
	i= segx
	while(i > GS.idealborder[0]) {
	    draw_set_color(0)
	...
	if(collision_rectangle(segx + 16, y - 4, GS.idealborder[0], y + 10, 739, 0, 1)) {
	    g= collision_rectangle(segx + 16, y - 4, GS.idealborder[0], y + 10, 739, 0, 1)
	    obj_heart.charge= 30
	    with(g) instance_destroy()
	    velocity.y+= 0.75
	    y+= 3
	...
	if(collision_rectangle(segx + 12, y + 4, GS.idealborder[0], y + 14, 743, 0, 1))
	    event_user(11)
	if(on == 1) {
	    if(segx > GS.idealborder[0] - 25) {
	        if(xm >= 0) xm= -2
	        xm-= 2
	        segx+= xm
	    } else  {
	        xm= 0
	        segx= GS.idealborder[0] - 25
	    }
	}
	if(on == 0) {
	    if(segx < segxinit) {
