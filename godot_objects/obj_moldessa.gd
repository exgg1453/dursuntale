# Auto-converted from GameMaker: obj_moldessa
...
func _gm_event_2_6():
	...
	if(mycommand < 75) GS.msg[0]= "Shh..."
	if(mycommand < 50) GS.msg[0]= "Hsh..."
	if(mycommand < 25) GS.msg[0]= "Krr..."
	GS.msg[1]= "%%%"
	GS.typer= 2
...
func _process(delta: float):
	...
	if(GS.mnfight == 2 and attacked == 0) {
	    GS.turntimer= 150
	    if(mercymod < 100 or scr_monstersum() >= 2) {
	    ...
	    if(mycommand >= 0)
	        GS.msg[0]= "* Moldessa\'s face rearranges&  it"
	    if(mycommand >= 40)
	        GS.msg[0]= "* Moldessa tries to make its&  face look like a movie-&  star\'s^1, but it shifts apart."
	    if(mycommand >= 70)
	        GS.msg[0]= "* Moldessa hides behind its ears^3.&* ... arms?"
	    if(mycommand >= 90)
	        GS.msg[0]= "* Smells like twelve-week-old&  gummy bears."
	    whatiheard= -1
	    if(GS.monsterhp[myself] < 15)
	        GS.msg[0]= "* Moldessa\'s face falls apart."
	    if(mercymod > 15)
	        GS.msg[0]= "* Moldessa\'s face looks... happy?"
	    attacked= 1
	}
	if(GS.myfight == 2 and whatiheard != -1 and GS.heard == 0) {
	    if(whatiheard == 0) {
	        GS.msc= 0
	        GS.msg[0]= "* MOLDESSA - ATK 27 DEF 23&* Can\'t decide on a face^1.&* Can\'t see friend from foe./^"
	        OBJ_WRITER.halt= 3
	    ...
	    if(whatiheard == 3) {
	        if(GS.goldreward[3] < 8)
	            GS.goldreward[3]+= 2
	        GS.msc= 0
	        GS.msg[0]= "* You encourage Moldessa to try&  a new look^1.&* Its face shifts.../^"
	    ...
	    if(whatiheard == 1) {
	        GS.msc= 0
	        GS.msg[0]= "* You adjust Moldessa\'s face./^"
	        if(mercymod < 100)
	            GS.msg[0]= "* You adjust Moldessa\'s face^1.&* It seems to be happy with its&  new look./^"
	        else  GS.msg[0]= "* You adjust Moldessa\'s face./^"
	    ...
	    if(whatiheard == 4) {
	        GS.msc= 0
	        if(GS.hp < GS.maxhp and lie == 0) {
	            lie= 1
	            GS.hp+= 2
	            snd_play(155/* snd_heal_c */)
	        }
	        if(scr_monstersum() >= 2) {
	            GS.msg[0]= "* You lie down and rest^1.&* Moldessa tucks you in with&  a blanket of moss./"
	            GS.msg[1]= "* While you were sleeping^1, the&  other monsters get bored./^"
	            pre_m= mercymod
	            // obj_monsterparent
	            with(605) {
